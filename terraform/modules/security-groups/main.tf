...
resource "aws_security_group" "api" {
  ...
  ingress {
    description = "HTTP - Nginx Soapbox frontend"
    from_port   = 80
    to_port     = 80
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
  ...
  ingress {
    description = "HTTPS - Nginx Soapbox frontend TLS"
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
  ...
  ingress {
    description = "HTTP API port (direct access, kept for internal/health-check use)"
    from_port   = 8080
    to_port     = 8080
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
